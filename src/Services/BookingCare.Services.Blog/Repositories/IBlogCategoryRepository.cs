...
{
    Task<IReadOnlyList<BlogCategoryEntity>> GetAllAsync(bool includeChildren, CancellationToken cancellationToken = default);
    Task<BlogCategoryEntity?> GetByIdAsync(Guid id, bool includeChildren = false, CancellationToken cancellationToken = default);
