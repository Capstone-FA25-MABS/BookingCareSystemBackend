...
public class DatabaseInitializationService
{
    ...
    {
        var scriptPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "..", "data", "db_auth.sql");

        var alternativePaths = new[]
        {
            Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "..", "data", "db_auth.sql"),
            Path.Combine(Directory.GetCurrentDirectory(), "data", "db_auth.sql"),
            "/app/data/db_auth.sql",
            Path.Combine("/app", "data", "db_auth.sql"),
            Path.Combine(AppContext.BaseDirectory, "data", "db_auth.sql")
        };
        ...
        if (foundScriptPath == null)
        {
            ...
            throw new FileNotFoundException($"SQL script file 'db_auth.sql' not found");
        }
    ...
    private string PreprocessSqlForAzureSqlEdge(string script)
    {
        ...
        script = Regex.Replace(script,
            @"CREATE DATABASE \[MABS_Auth\][^\n]*\r?\n(?:\s*CONTAINMENT[^\n]*\r?\n)?(?:\s*ON\s+PRIMARY[^\n]*\r?\n(?:\([^\)]+\)[^\n]*\r?\n)?)(?:\s*LOG ON[^\n]*\r?\n(?:\([^\)]+\)[^\n]*\r?\n)?)?(?:\s*WITH[^\n]*\r?\n)?",
            "CREATE DATABASE [MABS_Auth]\n",
            RegexOptions.IgnoreCase | RegexOptions.Multiline);

        script = Regex.Replace(script,
            @"ALTER DATABASE \[MABS_Auth\] SET COMPATIBILITY_LEVEL = 160",
            "ALTER DATABASE [MABS_Auth] SET COMPATIBILITY_LEVEL = 150",
            RegexOptions.IgnoreCase);
