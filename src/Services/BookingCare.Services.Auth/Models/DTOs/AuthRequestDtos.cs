...
public class RegisterRequest
{
    ...
    // Required for Patient only (Doctor password will be auto-generated)
    ...
    // Required for Patient and Staff only (validated in service by role)
    [RegularExpression(@"^0\d{9}$", ErrorMessage = "Phone number must be 10 digits starting with 0")]
    [StringLength(10, MinimumLength = 10, ErrorMessage = "Phone number must be exactly 10 digits")]
    public string? PhoneNumber { get; set; }
    ...
    public IEnumerable<ValidationResult> ValidateByRole(Role role)
    {
        ...
        {
            // Password is required for Patient (they register themselves)
            if (string.IsNullOrWhiteSpace(Password))
                yield return new ValidationResult("Password is required for Patient", new[] { nameof(Password) });

            if (string.IsNullOrWhiteSpace(ConfirmPassword))
                yield return new ValidationResult("ConfirmPassword is required for Patient", new[] { nameof(ConfirmPassword) });
            ...
            if (string.IsNullOrWhiteSpace(PhoneNumber))
                yield return new ValidationResult("PhoneNumber is required for Patient", new[] { nameof(PhoneNumber) });
        ...
        {
            // Password is NOT required for Doctor (will be auto-generated by system)
            // Password will be sent to doctor's email via message queue
        ...
        {
            if (string.IsNullOrWhiteSpace(PhoneNumber))
                yield return new ValidationResult("PhoneNumber is required for Staff", new[] { nameof(PhoneNumber) });
