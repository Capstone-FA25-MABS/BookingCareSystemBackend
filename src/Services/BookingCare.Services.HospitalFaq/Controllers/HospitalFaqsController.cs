using BookingCare.Services.HospitalFaq.Exceptions;
...
namespace BookingCare.Services.HospitalFaq.Controllers;
...
public class HospitalFaqsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetFaqs([FromQuery] HospitalFaqFilterRequest filter, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IActionResult> GetFaqsByHospitalId(Guid hospitalId, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IActionResult> GetFaq(Guid id, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IActionResult> CreateFaq([FromBody] CreateHospitalFaqRequest request, CancellationToken cancellationToken = default)
    {
    ...
    /// <summary>
    /// Update an existing FAQ (Staff only)
    /// </summary>
    [HttpPut("{id:guid}")]
    [Authorize(Roles = "Staff,Admin")]
    [ProducesResponseType(typeof(HospitalFaqResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> UpdateFaq(Guid id, [FromBody] UpdateHospitalFaqRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
