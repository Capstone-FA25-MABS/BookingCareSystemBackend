...
public class DatabaseInitializationService
{
    ...
    {
        var scriptPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "..", "data", "db_ai.sql");

        var alternativePaths = new[]
        {
            Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "..", "data", "db_ai.sql"),
            Path.Combine(Directory.GetCurrentDirectory(), "data", "db_ai.sql"),
            "/app/data/db_ai.sql",
            Path.Combine("/app", "data", "db_ai.sql"),
            Path.Combine(AppContext.BaseDirectory, "data", "db_ai.sql")
        };
        ...
        if (foundScriptPath == null)
        {
            ...
            throw new FileNotFoundException($"SQL script file 'db_ai.sql' not found");
        }
    ...
    private string PreprocessSqlForAzureSqlEdge(string script)
    {
        ...
        script = Regex.Replace(script,
            @"CREATE DATABASE \[MABS_AI\][^\n]*\r?\n(?:\s*CONTAINMENT[^\n]*\r?\n)?(?:\s*ON\s+PRIMARY[^\n]*\r?\n(?:\([^\)]+\)[^\n]*\r?\n)?)(?:\s*LOG ON[^\n]*\r?\n(?:\([^\)]+\)[^\n]*\r?\n)?)?(?:\s*WITH[^\n]*\r?\n)?",
            "CREATE DATABASE [MABS_AI]\n",
            RegexOptions.IgnoreCase | RegexOptions.Multiline);

        script = Regex.Replace(script,
            @"ALTER DATABASE \[MABS_AI\] SET COMPATIBILITY_LEVEL = 160",
            "ALTER DATABASE [MABS_AI] SET COMPATIBILITY_LEVEL = 150",
            RegexOptions.IgnoreCase);
