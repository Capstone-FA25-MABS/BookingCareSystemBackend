...
public class GeminiApiHelper
{
    ...
    {
        var requestBody = BuildRequestBody(prompt, temperature, maxOutputTokens);
        ...
        HandleErrorResponse(response, responseContent, model, apiVersion);

        var geminiResponse = ParseGeminiResponse(responseContent);
        var candidate = GetFirstCandidateOrThrow(geminiResponse, responseContent);

        HandleCandidateFinishReason(candidate);

        var generatedText = ExtractGeneratedTextOrThrow(candidate, responseContent);

        _logger.LogInformation(
            "Successfully called Gemini using {ApiVersion}/{Model}, generated {Length} characters",
            apiVersion,
            model,
    ...
    private object BuildRequestBody(string prompt, double temperature, int maxOutputTokens)
    {
        return new
        {
        ...
        string apiVersion)
    {
        ...
        _logger.LogWarning(
            "Gemini API failed: {StatusCode}, Response: {Response}",
            response.StatusCode,
            responseContent);
    ...
    private GeminiApiResponse ParseGeminiResponse(string responseContent)
    {
        return JsonSerializer.Deserialize<GeminiApiResponse>(
                   responseContent,
                   new JsonSerializerOptions { PropertyNameCaseInsensitive = true })
               ?? new GeminiApiResponse();
    }

    private Candidate GetFirstCandidateOrThrow(GeminiApiResponse geminiResponse, string responseContent)
    {
        if (geminiResponse.Candidates == null || geminiResponse.Candidates.Length == 0)
        {
            ...
            if (geminiResponse.PromptFeedback != null)
            {
                ...
                _logger.LogWarning(
                    "Prompt was blocked. BlockReason: {BlockReason}, SafetyRatings: {SafetyRatings}",
                    blockReason,
                    safetyRatings != null
                        ? string.Join(", ", safetyRatings.Select(r => $"{r.Category}:{r.Probability}"))
                        : "none");
    ...
    private void HandleCandidateFinishReason(Candidate candidate)
    {
        ...
        if (candidate.FinishReason == "SAFETY")
        {
            ...
            _logger.LogWarning(
                "Gemini API blocked response due to safety. SafetyRatings: {SafetyRatings}",
                safetyRatings != null
                    ? string.Join(", ", safetyRatings.Select(r => $"{r.Category}:{r.Probability}"))
                    : "none");
        ...
        {
            _logger.LogWarning(
                "Gemini API finished with reason: {FinishReason}",
                candidate.FinishReason);
        }
    }

    private string ExtractGeneratedTextOrThrow(Candidate candidate, string responseContent)
    {
        ...
        {
            logMessage +=
                $", SafetyRatings: {string.Join(", ", safetyRatings.Select(r => $"{r.Category}:{r.Probability}"))}";
        }
