...
public class DatabaseInitializationService
{
    ...
    public async Task InitializeAsync()
    {
        try
        {
            ...
            else
            {
                ...
                var context = scope.ServiceProvider.GetRequiredService<ServiceMedicalDbContext>();
    ...
    {
        var scriptPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "..", "data", "db_service-medical.sql");

        var alternativePaths = new[]
        {
            Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "..", "data", "db_service-medical.sql"),
            Path.Combine(Directory.GetCurrentDirectory(), "data", "db_service-medical.sql"),
            "/app/data/db_service-medical.sql",
            Path.Combine("/app", "data", "db_service-medical.sql"),
            Path.Combine(AppContext.BaseDirectory, "data", "db_service-medical.sql")
        };
        ...
        if (foundScriptPath == null)
        {
            ...
            throw new FileNotFoundException($"SQL script file 'db_service-medical.sql' not found");
        }
    ...
    private string PreprocessSqlForAzureSqlEdge(string script)
    {
        ...
        script = Regex.Replace(script,
            @"CREATE DATABASE \[MABS_ServiceMedical\][^\n]*\r?\n(?:\s*CONTAINMENT[^\n]*\r?\n)?(?:\s*ON\s+PRIMARY[^\n]*\r?\n(?:\([^\)]+\)[^\n]*\r?\n)?)(?:\s*LOG ON[^\n]*\r?\n(?:\([^\)]+\)[^\n]*\r?\n)?)?(?:\s*WITH[^\n]*\r?\n)?",
            "CREATE DATABASE [MABS_ServiceMedical]\n",
            RegexOptions.IgnoreCase | RegexOptions.Multiline);

        script = Regex.Replace(script,
            @"ALTER DATABASE \[MABS_ServiceMedical\] SET COMPATIBILITY_LEVEL = 160",
            "ALTER DATABASE [MABS_ServiceMedical] SET COMPATIBILITY_LEVEL = 150",
            RegexOptions.IgnoreCase);
