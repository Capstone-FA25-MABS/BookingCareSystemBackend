...
## 5. Get Refund Histories by Status
```http
...
## 6. Get Paged Refund Histories (Search)
```http
...
## 7. Update Refund History Status
```http
...
## 8. Delete Refund History (Only WAITING status)
```http
DELETE /api/v1.0/refundhistories/660e8400-e29b-41d4-a716-446655440001
```

## 9. Process Waiting Refunds (Batch Operation)
```http
...
## 11. Check if Payment Can Refund
```http
...
- **Staff**: Có th? xem t?t c?, update status, process batch
- **Admin**: Full access

### Performance Considerations
- Indexes ???c t?o cho: `user_id`, `status`, `payment_id` (unique)
- Composite index cho `(user_id, status)` ?? optimize queries
- Check constraints ensure data integrity

### Frontend Integration Tips
1. **Use the count field** t? get by user endpoint
