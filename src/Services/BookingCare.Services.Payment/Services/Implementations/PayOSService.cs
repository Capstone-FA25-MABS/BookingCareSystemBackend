...
public class PayOSService : BaseService, IPayOSService
{
    ...
    private void ValidateConfiguration()
    {
        ...
        LogInfo(
            "PayOS Configuration validated successfully - ClientId: {ClientId}",
            null,
    ...
    public async Task<PayOSPaymentResponse> CreatePaymentLinkAsync(PayOSPaymentRequest request)
    {
        return await ExecuteWithErrorHandling(
            ...
            {
                LogInfo(
                    "Creating PayOS payment link for PaymentId: {PaymentId}",
                    null,
                    request.PaymentId
                );
                ...
                {
                    items.AddRange(
                        request.Items.Select(item => new ItemData(
                            item.Name,
                            item.Quantity,
                            item.Price
                        ))
                ...
                // Save mapping PaymentId -> OrderCode
                // If this is a subscription payment, save subscription metadata
                ...
                {
                    // Regular payment (appointment)
                    await _mappingRepository.CreateMappingAsync(
                        request.PaymentId,
                        orderCode,
                        DateTime.UtcNow
                    );
                ...
                LogInfo(
                    "PayOS payment link created successfully - OrderCode: {OrderCode}, CheckoutUrl: {CheckoutUrl}",
                    null,
                    createResult.orderCode,
    ...
    /// </summary>
    public async Task<PayOSCallbackResponse> ProcessCallbackAsync(
        long orderCode,
        string code,
        bool cancel
    )
    {
        return await ExecuteWithErrorHandling(
            ...
            {
                LogInfo(
                    "Processing PayOS callback - OrderCode: {OrderCode}, Code: {Code}, Cancel: {Cancel}",
                    null,
                    orderCode,
                    code,
                    cancel
                ...
                // Process new payment
                return await ProcessNewPaymentAsync(
                    paymentId.Value,
                    payment,
                    orderCode,
                    code,
    ...
    /// </summary>
    private async Task<PayOSCallbackResponse> HandleMissingMappingAsync(
        long orderCode,
        string code,
        bool cancel
    )
    {
        LogWarning(
            "PayOS Callback - Cannot find PaymentId for OrderCode: {OrderCode} (mapping absent)",
            null,
            orderCode
        );
        ...
        {
            LogWarning(
                "PayOS Callback - Cannot retrieve payment info from PayOS API for OrderCode: {OrderCode}, Error: {Error}",
                null,
                orderCode,
                ex.Message
            );
    ...
    /// </summary>
    private PayOSCallbackResponse CreateFallbackResponse(
        long orderCode,
        string code,
        bool cancel,
        dynamic paymentInfo
    )
    {
        ...
        LogInfo(
            "PayOS Callback - Retrieved info from PayOS API - OrderCode: {OrderCode}, Status: {Status}, Amount: {Amount}",
            null,
            orderCode,
            paymentInfo.status,
            paymentInfo.amount
        ...
        return new PayOSCallbackResponse
        {
            ...
            Message = callbackSuccess
                ? "Payment processed successfully (mapping missing)"
                : "Payment processed but failed",
    ...
    private async Task<Models.DTOs.Responses.PaymentResponse> GetPaymentByIdAsync(Guid paymentId)
    {
        ...
        {
            LogWarning(
                "PayOS Callback - Payment not found in system for PaymentId: {PaymentId}",
                null,
                paymentId
            );
    ...
    /// </summary>
    private PayOSCallbackResponse CreateAlreadyProcessedResponse(
        Guid paymentId,
        Models.DTOs.Responses.PaymentResponse payment,
        long orderCode
    )
    {
        LogInfo(
            "PayOS Callback - Payment already processed - PaymentId: {PaymentId}, Status: {Status}",
            null,
            paymentId,
            payment.Status
        );
        ...
        return new PayOSCallbackResponse
        {
            ...
            Message =
                payment.Status == PaymentStatus.COMPLETED ? "Payment completed" : "Payment failed",
    ...
    /// </summary>
    private async Task<PayOSCallbackResponse> ProcessNewPaymentAsync(
        Guid paymentId,
        Models.DTOs.Responses.PaymentResponse payment,
        long orderCode,
        string code,
    ...
    )
    {
        ...
        LogInfo(
            "PayOS Callback - Successfully processed - PaymentId: {PaymentId}, Status: {Status}",
            null,
            paymentId,
            newStatus
        );
    ...
    /// </summary>
    private static (PaymentStatus Status, bool IsSuccess, string Message) DeterminePaymentOutcome(
        string code,
        bool cancel
    )
    {
    ...
    public async Task<object> GetPaymentInfoAsync(long orderCode)
    {
        return await ExecuteWithErrorHandling(
            async () =>
            {
                ...
                LogInfo(
                    "PayOS payment info retrieved successfully - OrderCode: {OrderCode}, Status: {Status}",
                    null,
                    orderCode,
                    paymentInfo.status
                );
    ...
    public async Task<bool> CancelPaymentLinkAsync(long orderCode, string cancellationReason = "")
    {
        return await ExecuteWithErrorHandling(
            ...
            {
                LogInfo(
                    "Cancelling PayOS payment link - OrderCode: {OrderCode}, Reason: {Reason}",
                    null,
                    orderCode,
                    cancellationReason
                );
                ...
                var cancelResult = await _payOS.cancelPaymentLink(
                    orderCode,
                ...
                LogInfo(
                    "PayOS payment link cancellation result - OrderCode: {OrderCode}, Success: {Success}",
                    null,
                    orderCode,
                    success
                );
    ...
    )?> GetSubscriptionMetadataAsync(long orderCode)
    {
        try
        {
            var mapping = await _mappingRepository.GetMappingByOrderCodeAsync(orderCode);

            if (mapping == null)
            {
                LogWarning("No mapping found for OrderCode: {OrderCode}", null, orderCode);
                return null;
            }

            // Check if this is a subscription payment
            if (
                !mapping.SubscriptionPlanId.HasValue
                || !mapping.HospitalId.HasValue
                || !mapping.IsSubscriptionUpgrade.HasValue
            )
            {
                return null;
            }
            ...
            LogInfo(
                "Retrieved subscription metadata for OrderCode: {OrderCode} - SubscriptionPlanId: {SubscriptionPlanId}, HospitalId: {HospitalId}, IsUpgrade: {IsUpgrade}, PlanType: {PlanType}",
                null,
                orderCode,
                mapping.SubscriptionPlanId,
                mapping.HospitalId,
                mapping.IsSubscriptionUpgrade.Value,
                mapping.PlanType
            );
            ...
            return (
                ...
                mapping.HospitalId,
                mapping.IsSubscriptionUpgrade.Value,
                mapping.CurrentHospitalSubscriptionId,
                mapping.PlanType
            );
        }
        catch (Exception ex)
        {
            LogError(
                ex,
                "Error getting subscription metadata for OrderCode: {OrderCode}",
                null,
                orderCode
            );
            return null;
