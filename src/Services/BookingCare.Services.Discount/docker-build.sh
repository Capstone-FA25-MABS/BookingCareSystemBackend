...
# Docker build and run script for Discount Service
...
run_container() {
    ...
    print_info "HTTP API: http://localhost:6007"
    print_info "gRPC: http://localhost:6017"
    print_info "Health Check: http://localhost:6007/api/discounts/health"
}
...
run_compose() {
    ...
    print_info "HTTP API: http://localhost:6007"
    print_info "gRPC: http://localhost:6017"
    print_info "Database: localhost:1434"
}
...
cleanup() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        docker volume rm discount_db_data 2>/dev/null || true
        docker volume rm discount_db_prod_data 2>/dev/null || true
...
test_service() {
    ...
    if curl -f -s "http://localhost:6007/api/discounts/health" > /dev/null; then
        print_success "Health check passed"
        
        # Show health response
        print_info "Health response:"
        curl -s "http://localhost:6007/api/discounts/health" | jq '.' 2>/dev/null || curl -s "http://localhost:6007/api/discounts/health"
    else
...
case "${2:-help}" in
    ...
    clean)
        ...
        ;;
    help|*)
        echo "Docker Management Script for Discount Service"
        echo
