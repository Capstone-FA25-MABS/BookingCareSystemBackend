...
public class DatabaseInitializationService
{
    ...
    {
        var scriptPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "..", "data", "db_content.sql");

        var alternativePaths = new[]
        {
            Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "..", "data", "db_content.sql"),
            Path.Combine(Directory.GetCurrentDirectory(), "data", "db_content.sql"),
            "/app/data/db_content.sql",
            Path.Combine("/app", "data", "db_content.sql"),
            Path.Combine(AppContext.BaseDirectory, "data", "db_content.sql")
        };
        ...
        if (foundScriptPath == null)
        {
            ...
            throw new FileNotFoundException($"SQL script file 'db_content.sql' not found");
        }
    ...
    private string PreprocessSqlForAzureSqlEdge(string script)
    {
        ...
        script = Regex.Replace(script,
            ...
            "ALTER DATABASE [MABS_Content] SET COMPATIBILITY_LEVEL = 150",
            RegexOptions.IgnoreCase);
