...
public class BlogRepository : IBlogRepository
{
    ...
    public BlogRepository(ContentDbContext dbContext, ILogger<BlogRepository> logger)
    {
        _dbContext = dbContext;
        _logger = logger;
    }

    public async Task<(IReadOnlyList<BlogEntity> Blogs, int TotalItems)> GetBlogsAsync(
    ...
    public async Task<IReadOnlyList<BlogEntity>> GetAllBlogsAsync(
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<BlogEntity?> GetByIdAsync(
        Guid id,
        CancellationToken cancellationToken = default)
    {
        return await _dbContext.Blogs
    ...
    public async Task<IReadOnlyList<BlogEntity>> GetRelatedBlogsAsync(
        Guid blogId,
        int limit = 10,
        CancellationToken cancellationToken = default)
    {
