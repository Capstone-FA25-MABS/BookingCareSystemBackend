...
public class HospitalFaqRepository : IHospitalFaqRepository
{
    ...
    public HospitalFaqRepository(ContentDbContext dbContext, ILogger<HospitalFaqRepository> logger)
    {
        _dbContext = dbContext;
        _logger = logger;
    }

    public async Task<(IReadOnlyList<HospitalFaqEntity> Faqs, int TotalItems)> GetFaqsAsync(
    ...
    public async Task<IReadOnlyList<HospitalFaqEntity>> GetFaqsByHospitalIdAsync(
        Guid hospitalId,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<HospitalFaqEntity?> GetByIdAsync(
        Guid id,
        CancellationToken cancellationToken = default)
    {
        return await _dbContext.HospitalFaqs
