...
public class AppointmentValidationService : BaseService, IAppointmentValidationService
{
    ...
    public async Task<AppointmentHistoryValidationResult> HasCompletedAppointmentWithDoctorAsync(Guid patientId, Guid doctorId)
    {
        return await ExecuteWithErrorHandling(async () =>
        {
            LogInfo("Validating appointment history - Patient: {PatientId}, Doctor: {DoctorId}",
            ...
            var request = new CheckPatientAppointmentHistoryRequest
            {
                ...
                ServiceId = string.Empty // Not checking service for doctor appointments
            };

            try
            {
                var response = await _appointmentClient.CheckPatientAppointmentHistoryAsync(request);
                return ProcessAppointmentHistoryResponse(response, "doctor", doctorId.ToString());
            }
    ...
    /// <returns>Appointment history validation result</returns>
    public async Task<AppointmentHistoryValidationResult> HasCompletedAppointmentWithServiceAsync(Guid patientId, Guid serviceId)
    {
        return await ExecuteWithErrorHandling(async () =>
  {
      LogInfo("Validating appointment history - Patient: {PatientId}, Service: {ServiceId}",
       null, patientId, serviceId);
      ...
      {
          var response = await _appointmentClient.CheckPatientAppointmentHistoryAsync(request);
          return ProcessAppointmentHistoryResponse(response, "service", serviceId.ToString());
      }
      catch (RpcException ex)
      {
  ...
  CheckPatientAppointmentHistoryResponse response,
        ...
        string targetId)
    {
        ...
        // Parse last appointment date if provided
        if (!string.IsNullOrEmpty(response.LastCompletedAppointmentDate))
        {
            if (DateTime.TryParse(response.LastCompletedAppointmentDate, out var lastDate))
            {
                result.LastCompletedAppointmentDate = lastDate;
            }
        }

        LogInfo("Appointment history validation result - {TargetType} {TargetId}: HasCompleted={HasCompleted}, Total={Total}",
            null, targetType, targetId, result.HasCompletedAppointment, result.TotalCompletedAppointments);
    ...
    /// </summary>
    private AppointmentHistoryValidationResult HandleGrpcException(RpcException ex, string targetType, string targetId)
    {
        ...
        LogWarning("Appointment validation failed for {TargetType} {TargetId} - allowing review creation as fallback",
               null, targetType, targetId);
