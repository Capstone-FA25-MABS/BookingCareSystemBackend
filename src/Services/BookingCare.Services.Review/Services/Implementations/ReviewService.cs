...
{
    public AccountEnrichmentService(
        AuthService.AuthServiceClient authClient,
        ILogger<AccountEnrichmentService> logger
    )
    ...
    public async Task<Dictionary<string, AccountInfo>> GetAccountDetailsAsync(
...
public class ReviewService : BaseService, IReviewService
{
    ...
    private async Task ValidateAppointmentHistoryAsync(CreateReviewRequest request)
    {
        ...
        if (request.TargetType == Enums.TargetType.DOCTOR && request.DoctorId.HasValue)
        {
            ...
            validationResult =
                await _appointmentValidationService.HasCompletedAppointmentWithDoctorAsync(
                    request.PatientId,
            ...
            {
                LogWarning(
                    "Patient {PatientId} attempted to review doctor {DoctorId} without completed appointment",
                    null,
                    request.PatientId,
                    request.DoctorId.Value
                );
            ...
            LogInfo(
                "Patient {PatientId} has {Count} completed appointments with doctor {DoctorId} - review allowed",
                null,
                request.PatientId,
                validationResult.TotalCompletedAppointments,
                request.DoctorId.Value
            );
        }
        else if (request.TargetType == Enums.TargetType.SERVICE && request.ServiceId.HasValue)
        {
            ...
            validationResult =
                await _appointmentValidationService.HasCompletedAppointmentWithServiceAsync(
                    request.PatientId,
            ...
            {
                LogWarning(
                    "Patient {PatientId} attempted to review service {ServiceId} without completed appointment",
                    null,
                    request.PatientId,
                    request.ServiceId.Value
                );
            ...
            LogInfo(
                "Patient {PatientId} has {Count} completed appointments with service {ServiceId} - review allowed",
                null,
                request.PatientId,
                validationResult.TotalCompletedAppointments,
                request.ServiceId.Value
    ...
    private async Task CheckForDuplicateReviewAsync(CreateReviewRequest request)
    {
        ...
        if (request.TargetType == Enums.TargetType.DOCTOR && request.DoctorId.HasValue)
        {
            existingReview = await _reviewRepository.GetExistingDoctorReviewAsync(
                request.PatientId,
                request.DoctorId.Value
            );
        ...
        {
            existingReview = await _reviewRepository.GetExistingServiceReviewAsync(
                request.PatientId,
                request.ServiceId.Value
            );
        ...
        {
            LogWarning(
                "Duplicate review attempt - Patient: {PatientId}, Target: {TargetName}, ExistingReview: {ExistingReviewId}",
                null,
                request.PatientId,
                targetName,
                existingReview.Id
    ...
    public async Task<PagedReviewsResponse> GetReviewsAsync(GetReviewsRequest request)
    {
        return await ExecuteWithErrorHandling(
            ...
            {
                LogInfo(
                    "Getting reviews with filters - PatientId: {PatientId}, DoctorId: {DoctorId}, ServiceId: {ServiceId}",
                    null,
                    request.PatientId?.ToString() ?? "null",
                    request.DoctorId?.ToString() ?? "null",
                    request.ServiceId?.ToString() ?? "null"
                );
                ...
                LogInfo(
                    "Retrieved {Count} reviews out of {Total} total",
                    null,
    ...
    )
    {
        return await ExecuteWithErrorHandling(
            async () =>
            {
                ...
                LogInfo(
                    "Getting reviews for doctor: {DoctorId} with optimized enrichment (minRating: {MinRating}, maxRating: {MaxRating})",
                    null,
                    doctorId,
                    minRating,
                    maxRating
                ...
                var pagedReviews = await _reviewRepository.GetReviewsByDoctorAsync(
                    doctorId,
                    page,
                    pageSize,
                    minRating,
                ...
                LogInfo(
                    "Retrieved {Count} reviews for doctor {DoctorId} with optimized enrichment",
                    null,
                    pagedReviews.Reviews.Count,
                    doctorId
                );
    ...
    )
    {
        return await ExecuteWithErrorHandling(
            async () =>
            {
                ...
                LogInfo(
                    "Getting reviews for service: {ServiceId} with optimized enrichment",
                    null,
                    serviceId
                );

                // Get reviews from repository
                var pagedReviews = await _reviewRepository.GetReviewsByServiceAsync(
                    serviceId,
                    page,
                    pageSize
                );
                ...
                LogInfo(
                    "Retrieved {Count} reviews for service {ServiceId} with optimized enrichment",
                    null,
                    pagedReviews.Reviews.Count,
                    serviceId
                );
    ...
    private async Task EnrichReviewsWithOptimizedInfo(List<ReviewResponse> reviews)
    {
        ...
        LogInfo(
            "Optimized enrichment: {PatientCount} patients via UserService, {ReplyAuthorCount} reply authors via AuthService",
            null,
            patientIds.Count,
            replyAuthorIds.Count
        );

        // Fetch enrichment data in parallel
        var (patientInfoDict, replyAuthorInfoDict) = await FetchEnrichmentDataAsync(
            patientIds,
            replyAuthorIds
        );
        ...
        LogInfo(
            "Successfully enriched {ReviewCount} reviews with optimized approach",
            null,
            reviews.Count
        );
    }
    ...
    private static (
        ...
        HashSet<string> replyAuthorIds
    ) CollectEnrichmentIds(List<ReviewResponse> reviews)
    {
    ...
    /// </summary>
    private static void EnrichPatientInfo(
    ...
    )
    {
        ...
        else
        {
            ...
            review.PatientInfo = new UserInfo { UserId = patientKey, Found = false };
        }
    }
    ...
    /// </summary>
    private static void EnrichReplyAuthorInfo(
    ...
    )
    {
        foreach (var reply in replies)
        {
            ...
            else
            {
                ...
                reply.AuthorInfo = new AccountInfo { AccountId = authorKey, Found = false };
            }
        }
    }
    ...
    )
    {
        return await ExecuteWithErrorHandling(
            async () =>
            {
                ...
                LogInfo(
                    "Retrieved {Count} reviews for hospital {HospitalId} with optimized enrichment",
                    null,
                    pagedReviews.Reviews.Count,
                    hospitalId
                );
            ...
            "GetReviewsByHospitalWithOptimizedEnrichment"
        );
    }
    ...
    public async Task<ReviewResponse> AddReplyAsync(AddReplyRequest request)
    {
        return await ExecuteWithErrorHandling(
            ...
            {
                LogInfo(
                    "Adding reply to review: {ReviewId} by author: {AuthorId}",
                    null,
                    request.ReviewId,
                    request.AuthorId
                );
                ...
                var updatedReview = await _reviewRepository.AddReplyAsync(
                    request.ReviewId,
                    replyEntity
                );
    ...
    public async Task<ReviewResponse> RemoveReplyAsync(string reviewId, string replyId)
    {
        return await ExecuteWithErrorHandling(
            ...
            {
                LogInfo(
                    "Removing reply: {ReplyId} from review: {ReviewId}",
                    null,
                    replyId,
                    reviewId
                );
    ...
    public async Task<ReviewResponse> UpdateReplyAsync(UpdateReplyRequest request)
    {
        return await ExecuteWithErrorHandling(
            ...
            {
                LogInfo(
                    "Updating reply: {ReplyId} in review: {ReviewId}",
                    null,
                    request.ReplyId,
                    request.ReviewId
                );
                ...
                var updatedReview = await _reviewRepository.UpdateReplyAsync(
                    request.ReviewId,
                    request.ReplyId,
                    request.Content
                );
    ...
    )
    {
        return await ExecuteWithErrorHandling(
            async () =>
            {
                ...
                LogInfo(
                    "Batch doctor statistics retrieved successfully for {Count} doctors",
                    null,
                    result.DoctorStatistics.Count
                );
                return result;
            },
    ...
    )
    {
        return await ExecuteWithErrorHandling(
            async () =>
            {
                ...
                LogInfo(
                    "Batch service statistics retrieved successfully for {Count} services",
                    null,
                    result.ServiceStatistics.Count
                );
                return result;
            },
    ...
    {
        ValidateGuid(hospitalId, nameof(hospitalId));
        return await _reviewRepository.GetAverageRatingByHospitalAsync(hospitalId);
    }
    ...
    {
        ValidateGuid(hospitalId, nameof(hospitalId));
        return await _reviewRepository.GetReviewCountByHospitalAsync(hospitalId);
    }
    ...
    {
        ValidateGuid(hospitalId, nameof(hospitalId));
        return await _reviewRepository.GetHospitalStatisticsAsync(hospitalId);
    }
    ...
    )
    {
        return await ExecuteWithErrorHandling(
            async () =>
            {
                ...
                LogInfo("Getting detailed statistics for hospital: {HospitalId}", null, hospitalId);

                var result = await _reviewRepository.GetHospitalDetailedStatisticsAsync(hospitalId);

                LogInfo(
                    "Detailed statistics retrieved for hospital {HospitalId}: Avg={Avg}, Total={Total}",
                    null,
                    hospitalId,
    ...
    )
    {
        return await ExecuteWithErrorHandling(
            ...
            {
                LogInfo(
                    "Getting batch statistics for {Count} hospitals",
                    null,
                    request.HospitalIds.Count
                );

                // ✅ ValidationFilter đã handle tất cả validation rồi

                // Remove duplicates (business logic)
                var uniqueHospitalIds = request.HospitalIds.Distinct().ToList();

                var result = await _reviewRepository.GetBatchHospitalsStatisticsAsync(
                    uniqueHospitalIds
                );

                LogInfo(
                    "Batch hospital statistics retrieved successfully for {Count} hospitals",
                    null,
                    result.HospitalStatistics.Count
                );
                return result;
            },
